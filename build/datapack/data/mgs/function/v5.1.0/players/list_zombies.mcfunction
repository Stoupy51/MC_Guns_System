...
data modify storage mgs:temp dialog set value {type:"minecraft:multi_action",title:[{text:"👥 ",color:"dark_green",bold:true}, {translate:"mgs.manage_players"}],body:[{type:"minecraft:plain_message",contents:{translate:"mgs.click_a_player_to_assign_them_a_team",color:"gray"}}],actions:[],columns:3,pause:false,after_action:"none",exit_action:{label:[{text:"◀ ",color:"gray"}, {translate:"mgs.back"}],tooltip:{translate:"mgs.return_to_setup"},action:{type:"show_dialog",dialog:"mgs:zombies/setup"}}}
...
execute unless data storage mgs:temp dialog.actions[0] run data modify storage mgs:temp dialog.actions append value {label:{translate:"mgs.no_players_online",color:"red"},tooltip:{translate:"mgs.nobody_to_manage"},action:{type:"show_dialog",dialog:"mgs:zombies/setup"}}
