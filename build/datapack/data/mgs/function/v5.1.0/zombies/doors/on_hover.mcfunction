...
execute unless score #door_partial mgs.data matches 1.. run data modify storage smithed.actionbar:input message set value {json:[{"text":"🛠 "},{"storage":"mgs:temp","nbt":"_door_hover_name","color":"yellow","interpret":true},[{"text":" - ","color":"gray"}, {"translate":"mgs.cost_2"}],{"score":{"name":"#door_price","objective":"mgs.data"},"color":"yellow"},[{"text":" ","color":"gray"}, {"translate":"mgs.points_2"}]],priority:"conditional",freeze:5}
