...
#> mgs:v5.1.0/zombies/tag_spawns_near_players
#
...
execute as @a[scores={mgs.zb.in_game=1},gamemode=!spectator] at @s run function mgs:v5.1.0/zombies/tag_zb_near_32

# Second pass: 64 blocks if none found
execute if score #zb_near_found mgs.data matches 0 as @a[scores={mgs.zb.in_game=1},gamemode=!spectator] at @s run function mgs:v5.1.0/zombies/tag_zb_near_64
