...
execute positioned ~ ~-1 ~ if block ~ ~ ~ air run return run function mgs:v5.1.0/zombies/barriers/destroyed_tick
...
execute if score @s mgs.zb.barrier.rp_timer matches 1.. run function mgs:v5.1.0/zombies/barriers/handle_repair with storage mgs:temp _brptick
execute if score @s mgs.zb.barrier.rp_timer matches 0 if score @s mgs.zb.barrier.state matches 1 run function mgs:v5.1.0/zombies/barriers/find_repairer with storage mgs:temp _brptick
