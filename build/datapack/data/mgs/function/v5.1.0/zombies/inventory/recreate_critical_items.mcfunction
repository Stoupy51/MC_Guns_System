
#> mgs:v5.1.0/zombies/inventory/recreate_critical_items
#
# @within	???
#

execute unless items entity @s hotbar.0 *[custom_data~{mgs:{knife:true,zombies:{hotbar:0}}}] run item replace entity @s hotbar.0 with minecraft:iron_sword[unbreakable={},custom_data={mgs:{knife:true}},item_name={"translate":"mgs.knife","color":"white","italic":false},attribute_modifiers=[{type:"attack_damage",amount:20,operation:"add_value",slot:"mainhand",id:"minecraft:base_attack_damage"},{type:"attack_speed",amount:-2.5,operation:"add_value",slot:"mainhand",id:"minecraft:base_attack_speed"}]]
execute unless items entity @s hotbar.0 *[custom_data~{mgs:{knife:true,zombies:{hotbar:0}}}] run function mgs:v5.1.0/zombies/inventory/apply_slot_tag {slot:"hotbar.0",group:"hotbar",index:0}

execute unless items entity @s hotbar.7 *[custom_data~{mgs:{gun:true,zombies:{hotbar:7}}}] run function mgs:v5.1.0/zombies/inventory/loot_replace_lethal
execute unless items entity @s hotbar.7 *[custom_data~{mgs:{gun:true,zombies:{hotbar:7}}}] run function mgs:v5.1.0/zombies/inventory/apply_slot_tag {slot:"hotbar.7",group:"hotbar",index:7}

execute unless items entity @s hotbar.8 *[custom_data~{mgs:{zb_info:true,zombies:{hotbar:8}}}] run function mgs:v5.1.0/zombies/inventory/refresh_info_item

