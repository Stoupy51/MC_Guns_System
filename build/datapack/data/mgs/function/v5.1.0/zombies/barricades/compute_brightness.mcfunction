...
#> mgs:v5.1.0/zombies/barricades/compute_brightness
#
# @executed	at @s
#
# @within	mgs:v5.1.0/zombies/game_tick [ at @s ]
#			mgs:v5.1.0/zombies/barricades/setup_iter [ as @n[tag=mgs._barricade_new_d] & at @s ]
#

# Reset light score, then sample own position and all 6 neighbors (stop early at 15)
scoreboard players set #light mgs.data 0
execute if score #light mgs.data matches ..14 positioned ~ ~ ~ run function mgs:v5.1.0/zombies/barricades/check_light
execute if score #light mgs.data matches ..14 positioned ~ ~1 ~ run function mgs:v5.1.0/zombies/barricades/check_light
execute if score #light mgs.data matches ..14 positioned ~ ~-1 ~ run function mgs:v5.1.0/zombies/barricades/check_light
execute if score #light mgs.data matches ..14 positioned ~1 ~ ~ run function mgs:v5.1.0/zombies/barricades/check_light
execute if score #light mgs.data matches ..14 positioned ~-1 ~ ~ run function mgs:v5.1.0/zombies/barricades/check_light
execute if score #light mgs.data matches ..14 positioned ~ ~ ~1 run function mgs:v5.1.0/zombies/barricades/check_light
