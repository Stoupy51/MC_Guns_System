...
execute at @n[tag=bs.interaction.target] if entity @n[tag=mgs.wunderfizz_orb,distance=..3,scores={mgs.zb.wf.anim=..0}] if score @s mgs.zb.wf_pid = @n[tag=mgs.wunderfizz_orb,distance=..3] mgs.zb.wf.buyer run return run function mgs:v5.1.0/zombies/wunderfizz/hover_result
