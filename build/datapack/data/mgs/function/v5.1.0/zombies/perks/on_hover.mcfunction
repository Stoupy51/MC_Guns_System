...
execute unless score #pk_partial mgs.data matches 1.. run data modify storage smithed.actionbar:input message set value {json:[{"text":"🥤 "},{"storage":"mgs:temp","nbt":"_pk_hover_name","color":"light_purple","interpret":true},[{"text":" - ","color":"gray"}, {"translate":"mgs.cost_2"}],{"score":{"name":"#pk_price","objective":"mgs.data"},"color":"yellow"},[{"text":" ","color":"gray"}, {"translate":"mgs.points_2"}]],priority:"conditional",freeze:5}
