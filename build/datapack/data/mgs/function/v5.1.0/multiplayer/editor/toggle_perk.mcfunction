...
#
# @within	mgs:v5.1.0/multiplayer/editor/pick_perk with storage mgs:temp
#
# @args		_toggle_perk (unknown)
#

# Already selected → remove it (recompute refunds automatically)
$execute if data storage mgs:temp editor{perks:["$(_toggle_perk)"]} run return run function mgs:v5.1.0/multiplayer/editor/remove_perk
