...
execute if score #snd_bomb_state mgs.data matches 0 as @a[tag=mgs.snd_alive,predicate=mgs:v5.1.0/is_sneaking,gamemode=!spectator] at @s if entity @e[tag=mgs.snd_obj,distance=..3] run function mgs:v5.1.0/multiplayer/gamemodes/snd/try_plant
...
scoreboard players set #snd_channeling mgs.data 0
execute if score #snd_bomb_state mgs.data matches 2 as @a[tag=mgs.snd_alive,predicate=mgs:v5.1.0/is_sneaking,gamemode=!spectator] at @s if entity @e[tag=mgs.snd_bomb,distance=..3] run function mgs:v5.1.0/multiplayer/gamemodes/snd/try_defuse
