...
#> mgs:v5.1.0/missions/tp_to_spawn
#
# @executed	as @n[tag=mgs.spawn_candidate,sort=random]
#
# @within	mgs:v5.1.0/missions/pick_spawn [ as @n[tag=mgs.spawn_candidate,sort=random] ]
#
...
execute as @p[tag=mgs.spawn_pending] run function mgs:v5.1.0/missions/tp_player_at with storage mgs:temp _tp
