...
#
# @within	string in mgs:v5.1.0/utils/update_all_lore {slot:"weapon.mainhand"}
#
# @args		slot (string)
#

# Rebuild all lore lines for the weapon in the given slot from its current stats
# Usage: function mgs:v5.1.0/utils/update_all_lore {slot:"weapon.mainhand"}

# Tag player for identification
tag @s add mgs.update_lore

# Read stats from item into scores
$execute summon item_display run function mgs:v5.1.0/lore/extract_stats {"slot":"$(slot)"}
...
function mgs:v5.1.0/lore/compute_values

# Build new lore based on weapon type
execute if score #is_grenade mgs.data matches 1 run function mgs:v5.1.0/lore/build_grenade with storage mgs:input lore
execute if score #is_grenade mgs.data matches 0 run function mgs:v5.1.0/lore/build_gun with storage mgs:input lore
...
$execute summon item_display run function mgs:v5.1.0/lore/apply {"slot":"$(slot)"}
