...
$summon minecraft:interaction $(x) $(y) $(z) {width:0.6f,height:0.6f,response:true,Tags:["mgs.power_switch","mgs.gm_entity","bs.entity.interaction","mgs.pw_face_$(facing)","_pw_new"]}
...
execute as @e[tag=_pw_new] run function #bs.interaction:on_hover_enter {run:"function mgs:v5.0.0/zombies/power/on_hover_enter",executor:"source"}
execute as @e[tag=_pw_new] run function #bs.interaction:on_hover_leave {run:"function mgs:v5.0.0/zombies/power/on_hover_leave",executor:"source"}
