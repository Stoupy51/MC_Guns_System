...
execute store result score #_pk_power mgs.data run scoreboard players get @n[tag=bs.interaction.target] mgs.zb.perk.power
execute if score #_pk_power mgs.data matches 1 unless score #zb_power mgs.data matches 1 run return run tellraw @s [[{"text":"","color":"gold"},"[",{"translate": "mgs"},"] "],{"translate": "mgs.requires_power","color":"red"}]
...
execute if score #_pk_owned mgs.data matches 1 run return run tellraw @s [[{"text":"","color":"gold"},"[",{"translate": "mgs"},"] "],{"translate": "mgs.already_have_this_perk","color":"yellow"}]
...
execute unless score @s mgs.zb.points >= #_pk_price mgs.data run return run tellraw @s [[{"text":"","color":"gold"},"[",{"translate": "mgs"},"] "],{"translate": "mgs.not_enough_points","color":"red"}]

# Deduct points
scoreboard players operation @s mgs.zb.points -= #_pk_price mgs.data
