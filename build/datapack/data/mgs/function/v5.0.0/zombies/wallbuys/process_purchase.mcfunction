...
$execute if score #wb_purchase_done mgs.data matches 0 unless items entity @s hotbar.1 *[custom_data~{mgs:{gun:true}}] run function mgs:v5.0.0/zombies/wallbuys/give_to_slot {hotbar:1,inventory:1,weapon_id:"$(weapon_id)",magazine_id:"$(magazine_id)"}
$execute if score #wb_purchase_done mgs.data matches 0 unless items entity @s hotbar.2 *[custom_data~{mgs:{gun:true}}] run function mgs:v5.0.0/zombies/wallbuys/give_to_slot {hotbar:2,inventory:2,weapon_id:"$(weapon_id)",magazine_id:"$(magazine_id)"}
$execute if score #wb_purchase_done mgs.data matches 0 unless items entity @s hotbar.3 *[custom_data~{mgs:{gun:true}}] if score @s mgs.zb.perk.mule_kick matches 1.. run function mgs:v5.0.0/zombies/wallbuys/give_to_slot {hotbar:3,inventory:3,weapon_id:"$(weapon_id)",magazine_id:"$(magazine_id)"}
