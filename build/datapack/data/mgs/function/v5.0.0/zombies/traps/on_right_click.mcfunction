...
execute if score #trap_power mgs.data matches 1 unless score #zb_power mgs.data matches 1 run return run tellraw @s [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],[{"text":" ⚡ ","color":"red"}, {"translate":"mgs.requires_power"}]]
...
execute unless score #trap_ready mgs.data matches 1 run return run tellraw @s [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],[{"text":" ","color":"yellow"}, {"translate":"mgs.trap_not_ready"}]]

# Check price
execute store result score #trap_price mgs.data run scoreboard players get @n[tag=bs.interaction.target] mgs.zb.trap.price
execute unless score @s mgs.zb.points >= #trap_price mgs.data run return run tellraw @s [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],[{"text":" ","color":"red"}, {"translate":"mgs.not_enough_points"}]]
...
# Announce
tellraw @a[scores={mgs.zb.in_game=1}] [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],[{"text":" ⚠ ","color":"gold"}, {"translate":"mgs.trap_activated"}]]
