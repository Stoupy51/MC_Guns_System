...
$data modify storage mgs:temp dialog.actions append value {label:{text:"👁",color:"aqua"},width:20,tooltip:["",{translate: "mgs.toggle_to",color:"white"},{translate: "mgs.private",color:"red"}],action:{type:"run_command",command:"/trigger mgs.player.config set $(vis_trig)"}}
