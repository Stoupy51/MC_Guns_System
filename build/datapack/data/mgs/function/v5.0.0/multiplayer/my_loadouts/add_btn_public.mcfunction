...
$data modify storage mgs:temp dialog.actions append value {label:{translate: "mgs.public_private",color:"red"},tooltip:{translate: "mgs.toggle_this_loadout_to_private",color:"red"},action:{type:"run_command",command:"/trigger mgs.player.config set $(vis_trig)"}}
