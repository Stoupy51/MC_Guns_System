...
execute store result score #_tick_mod mgs.data run scoreboard players get #mp_timer mgs.data
scoreboard players operation #_tick_mod mgs.data %= #20 mgs.data
execute if score #_tick_mod mgs.data matches 0 run function mgs:v5.0.0/multiplayer/timer_display
...
execute as @a[scores={mgs.mp.in_game=1},gamemode=!creative,gamemode=!spectator] at @s run function mgs:v5.0.0/multiplayer/check_bounds

# ── Out-of-bounds check ──
execute as @a[scores={mgs.mp.in_game=1},gamemode=!creative,gamemode=!spectator] at @s if entity @e[tag=mgs.oob_point,distance=..5] run function mgs:v5.0.0/multiplayer/oob_kill
