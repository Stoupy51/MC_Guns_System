...
execute if entity @s[tag=mgs.element.wallbuy] run scoreboard players add #_yaw mgs.data 180
...
execute if entity @s[tag=mgs.element.door] as @p[tag=mgs.map_editor] run data modify storage mgs:temp _zb_offhand_block set from entity @s Inventory[{Slot:-106b}].id
execute if entity @s[tag=mgs.element.door] if data storage mgs:temp _zb_offhand_block run execute as @n[tag=mgs.new_zb_marker] run data modify entity @s data.block set from storage mgs:temp _zb_offhand_block
execute if entity @s[tag=mgs.element.door] unless data storage mgs:temp _zb_offhand_block run tellraw @a[tag=mgs.map_editor] [[{"text":"","color":"gold"},"[",{"translate": "mgs"},"] "],{"translate": "mgs.door_placed_without_block_hold_a_block_in_offhand","color":"yellow"}]
