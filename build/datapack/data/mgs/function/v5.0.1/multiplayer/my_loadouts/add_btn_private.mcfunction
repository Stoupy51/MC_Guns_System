...
#> mgs:v5.0.1/multiplayer/my_loadouts/add_btn_private
#
...
$data modify storage mgs:temp dialog.actions append value {label:{translate:"mgs.private_public",color:"aqua"},tooltip:{translate:"mgs.toggle_this_loadout_to_public",color:"green"},action:{type:"run_command",command:"/trigger mgs.player.config set $(vis_trig)"}}
$data modify storage mgs:temp dialog.actions append value {label:[{text:"",color:"gold"},"✏ ",{translate:"mgs.edit"}],tooltip:{translate:"mgs.re_run_the_loadout_wizard_saving_overwrites_this_loadout",color:"yellow"},action:{type:"run_command",command:"/trigger mgs.player.config set $(edit_trig)"}}
