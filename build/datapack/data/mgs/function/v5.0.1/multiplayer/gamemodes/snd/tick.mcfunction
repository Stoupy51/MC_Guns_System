...
execute if score #snd_round_timer mgs.data matches ..0 if score #snd_bomb_state mgs.data matches ..1 run function mgs:v5.0.1/multiplayer/gamemodes/snd/defenders_win
...
execute if score #snd_atk_alive mgs.data matches 0 if score #snd_bomb_state mgs.data matches ..1 run function mgs:v5.0.1/multiplayer/gamemodes/snd/defenders_win
...
execute if score #snd_bomb_state mgs.data matches 0 as @a[tag=mgs.snd_alive,predicate=mgs:v5.0.1/is_sneaking] at @s if entity @e[tag=mgs.snd_obj,distance=..3] run function mgs:v5.0.1/multiplayer/gamemodes/snd/try_plant

# Check defusing (defender near bomb and sneaking)
execute if score #snd_bomb_state mgs.data matches 2 as @a[tag=mgs.snd_alive,predicate=mgs:v5.0.1/is_sneaking] at @s if entity @e[tag=mgs.snd_bomb,distance=..3] run function mgs:v5.0.1/multiplayer/gamemodes/snd/try_defuse
