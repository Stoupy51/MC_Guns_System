...
#> mgs:v5.0.1/multiplayer/gamemodes/snd/next_round
#
# @within	mgs:v5.0.1/multiplayer/gamemodes/snd/attackers_win
#			mgs:v5.0.1/multiplayer/gamemodes/snd/defenders_win
#
...
execute if score #snd_red_wins mgs.data >= #snd_win_threshold mgs.data run return run function mgs:v5.0.1/multiplayer/team_wins {team:"Red"}
execute if score #snd_blue_wins mgs.data >= #snd_win_threshold mgs.data run return run function mgs:v5.0.1/multiplayer/team_wins {team:"Blue"}
...
execute if score #snd_round mgs.data matches 4 run tellraw @a [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],[{"text":"⚔ ","color":"gold"}, {"translate":"mgs.sides_swapped"}]]
...
schedule function mgs:v5.0.1/multiplayer/gamemodes/snd/start_round 60t
