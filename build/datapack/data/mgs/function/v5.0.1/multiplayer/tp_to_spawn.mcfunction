...
#> mgs:v5.0.1/multiplayer/tp_to_spawn
#
# @executed	as @e[tag=mgs.spawn_candidate,sort=random]
#
# @within	mgs:v5.0.1/multiplayer/pick_spawn [ as @e[tag=mgs.spawn_candidate,sort=random] ]
#			mgs:v5.0.1/multiplayer/pick_spawn_random [ as @n[tag=mgs.spawn_candidate,sort=random] ]
#
...
execute as @p[tag=mgs.spawn_pending] run function mgs:v5.0.1/multiplayer/tp_player_at with storage mgs:temp _tp
