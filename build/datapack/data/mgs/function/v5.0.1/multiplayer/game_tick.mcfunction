...
execute if score #tick_mod mgs.data matches 0 run function mgs:v5.0.1/multiplayer/timer_display
...
execute as @e[type=player,scores={mgs.mp.in_game=1,mgs.mp.death_count=0},gamemode=!creative,gamemode=!spectator] at @s if entity @e[tag=mgs.oob_point,distance=..5] run function mgs:v5.0.1/multiplayer/oob_kill
...
execute if data storage mgs:multiplayer game{gamemode:"tdm"} run function mgs:v5.0.1/multiplayer/gamemodes/tdm/tick
execute if data storage mgs:multiplayer game{gamemode:"dom"} run function mgs:v5.0.1/multiplayer/gamemodes/dom/tick
