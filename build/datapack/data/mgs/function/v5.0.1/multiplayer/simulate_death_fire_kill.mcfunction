...
#> mgs:v5.0.1/multiplayer/simulate_death_fire_kill
#
# @executed	at @s
#
# @within	mgs:v5.0.1/multiplayer/simulate_death with storage mgs:input with
#
...
execute unless entity @a[tag=mgs.temp_killer] run return run function mgs:v5.0.1/multiplayer/random_self_kill_message

# Normal kill: fire signal and show message
tag @s add mgs.temp_victim
$execute as $(attacker) run function #mgs:signals/on_kill
function mgs:v5.0.1/multiplayer/random_kill_message
