...
execute if score #mp_has_boundary mgs.data matches 1 run function mgs:v5.0.1/shared/load_bounds {mode:"multiplayer"}

# Summon out-of-bounds markers
function mgs:v5.0.1/shared/summon_oob {mode:"multiplayer"}

# Summon spawn point markers (for smart spawn selection)
function mgs:v5.0.1/multiplayer/summon_spawns
...
execute if data storage mgs:multiplayer game{gamemode:"ffa"} run function mgs:v5.0.1/multiplayer/gamemodes/ffa/setup
execute if data storage mgs:multiplayer game{gamemode:"tdm"} run function mgs:v5.0.1/multiplayer/gamemodes/tdm/setup
execute if data storage mgs:multiplayer game{gamemode:"dom"} run function mgs:v5.0.1/multiplayer/gamemodes/dom/setup
execute if data storage mgs:multiplayer game{gamemode:"hp"} run function mgs:v5.0.1/multiplayer/gamemodes/hp/setup
execute if data storage mgs:multiplayer game{gamemode:"snd"} run function mgs:v5.0.1/multiplayer/gamemodes/snd/setup
...
execute if data storage mgs:multiplayer game{gamemode:"ffa"} run function mgs:v5.0.1/multiplayer/create_sidebar_ffa
execute if data storage mgs:multiplayer game{gamemode:"tdm"} run function mgs:v5.0.1/multiplayer/create_sidebar_team {title:"Team Deathmatch"}
execute if data storage mgs:multiplayer game{gamemode:"dom"} run function mgs:v5.0.1/multiplayer/create_sidebar_dom
execute if data storage mgs:multiplayer game{gamemode:"hp"} run function mgs:v5.0.1/multiplayer/create_sidebar_hp
execute if data storage mgs:multiplayer game{gamemode:"snd"} run function mgs:v5.0.1/multiplayer/create_sidebar_team {title:"Search & Destroy"}

# Show kills in player list (tab)
scoreboard objectives setdisplay list mgs.mp.kills

# Teleport players to spawn points
function mgs:v5.0.1/multiplayer/tp_all_to_spawns
...
execute as @a[scores={mgs.mp.in_game=1}] at @s unless score @s mgs.mp.class matches 0 run function mgs:v5.0.1/multiplayer/apply_class

# For players with no class: auto-apply default custom loadout if set
scoreboard players add @s mgs.mp.class 0
execute as @a[scores={mgs.mp.in_game=1}] at @s if score @s mgs.mp.class matches 0 if score @s mgs.mp.default matches 1.. run function mgs:v5.0.1/multiplayer/auto_apply_default

# Show class selection dialog to EVERYONE (so they can change during prep)
execute as @a[scores={mgs.mp.in_game=1}] run function mgs:v5.0.1/multiplayer/select_class
...
schedule function mgs:v5.0.1/multiplayer/end_prep 200t
