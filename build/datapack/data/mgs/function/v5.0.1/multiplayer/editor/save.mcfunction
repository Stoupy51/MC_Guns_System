...
execute unless data storage mgs:temp editor{equip_slot1:""} run function mgs:v5.0.1/multiplayer/editor/append_equip1 with storage mgs:temp editor
execute unless data storage mgs:temp editor{equip_slot2:""} run function mgs:v5.0.1/multiplayer/editor/append_equip2 with storage mgs:temp editor
...
execute if score #pmag_count mgs.data matches 1.. run function mgs:v5.0.1/multiplayer/editor/append_mag_slots

# 5. Secondary magazine slots (continuing from #inv_slot)
execute if data storage mgs:temp _build.secondary_data run function mgs:v5.0.1/multiplayer/editor/start_secondary_mags

# Auto-name the loadout and set gun display names
function mgs:v5.0.1/multiplayer/editor/set_name with storage mgs:temp editor
function mgs:v5.0.1/multiplayer/editor/set_main_gun_display with storage mgs:temp editor
