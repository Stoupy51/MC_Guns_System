...
#> mgs:v5.0.1/multiplayer/editor/append_mag_slots
#
# @executed	as @e[type=player,sort=random] & at @s
#
# @within	mgs:v5.0.1/multiplayer/editor/save
#			mgs:v5.0.1/multiplayer/editor/start_secondary_mags
#
...
execute if data storage mgs:temp _mag_data{mag_consumable:1b} run function mgs:v5.0.1/multiplayer/editor/append_mag_consumable
execute if data storage mgs:temp _mag_data{mag_consumable:1b} run return 0

# Non-consumable: add one slot per count
execute if score #pmag_count mgs.data matches 1.. run function mgs:v5.0.1/multiplayer/editor/append_mag_loop
