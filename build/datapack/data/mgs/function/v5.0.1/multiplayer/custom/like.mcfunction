...
execute if data storage mgs:temp _pd_src[0] run function mgs:v5.0.1/multiplayer/custom/like_pd_rebuild

# Step 2: If not already liked, increment like counter on the loadout
execute if score #already_liked mgs.data matches 0 run function mgs:v5.0.1/multiplayer/custom/like_increment_setup
...
function mgs:v5.0.1/multiplayer/marketplace/browse
