...
execute as @a[scores={mgs.mi.in_game=1,mgs.mp.spectate_timer=0},gamemode=spectator] at @s run function mgs:v5.0.1/missions/actual_respawn

# Increment mission timer
scoreboard players add #mi_timer mgs.data 1

# Boundary enforcement (skip spectators) & OOB Check
execute if score #mi_has_boundary mgs.data matches 1 as @e[tag=mgs.mission_enemy] at @s run function mgs:v5.0.1/missions/check_bounds
execute if score #mi_has_boundary mgs.data matches 1 as @e[type=player,scores={mgs.mi.in_game=1},gamemode=!creative,gamemode=!spectator] at @s run function mgs:v5.0.1/missions/check_bounds
