...
function mgs:v5.0.1/shared/load_base_coordinates {mode:"missions"}
...
execute if score #mi_has_boundary mgs.data matches 1 run function mgs:v5.0.1/shared/load_bounds {mode:"missions"}

# Forceload the mission area to ensure chunks are loaded
execute if score #mi_has_boundary mgs.data matches 1 run function mgs:v5.0.1/shared/forceload_area
...
execute as @a[scores={mgs.mi.in_game=1}] run function mgs:v5.0.1/shared/tp_to_position with storage mgs:temp _tp

# Schedule preload completion after 1 second
schedule function mgs:v5.0.1/missions/preload_complete 20t
