...
execute if score @s mgs.special.infinite_ammo matches 1.. run return run function mgs:v5.0.1/ammo/infinite_refill

# Remove 1 bullet from player's ammo count
scoreboard players remove @s mgs.remaining_bullets 1
execute if score @s mgs.remaining_bullets matches ..0 run function mgs:v5.0.1/ammo/reload
