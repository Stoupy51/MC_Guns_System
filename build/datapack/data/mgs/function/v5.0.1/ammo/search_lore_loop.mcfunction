...
#> mgs:v5.0.1/ammo/search_lore_loop
#
# @executed	as @e[type=player,sort=random] & at @s
#
# @within	mgs:v5.0.1/ammo/modify_lore {"slot":"$(slot)"}
#			mgs:v5.0.1/ammo/search_lore_loop {"slot":"$(slot)"}
#
# @args		slot (string)
#
...
execute if score #success mgs.data matches 1 summon item_display run return run function mgs:v5.0.1/ammo/found_lore_line with storage mgs:input with
...
execute if score #success mgs.data matches 1 run data modify storage mgs:input with.capacity set from storage mgs:temp components."minecraft:custom_data".mgs.stats.capacity
$execute if score #success mgs.data matches 1 run data modify storage mgs:input with.slot set value "$(slot)"
execute if score #success mgs.data matches 1 summon item_display run return run function mgs:v5.0.1/ammo/found_line with storage mgs:input with
