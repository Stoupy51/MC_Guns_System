...
#> mgs:v5.0.1/ammo/search_mag_lore_loop
#
# @executed	as @e[type=player,sort=random] & at @s
#
# @within	mgs:v5.0.1/ammo/modify_mag_lore {"slot":"$(slot)"}
#			mgs:v5.0.1/ammo/search_mag_lore_loop {"slot":"$(slot)"}
#
# @args		slot (string)
#
...
execute if score #success mgs.data matches 1 summon item_display run return run function mgs:v5.0.1/ammo/found_mag_lore_line with storage mgs:input with

# Continue searching if not found
data remove storage mgs:temp copy[0]
scoreboard players add #index mgs.data 1
$execute if data storage mgs:temp copy[0] run function mgs:v5.0.1/ammo/search_mag_lore_loop {"slot":"$(slot)"}
