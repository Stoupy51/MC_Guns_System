...
#> mgs:v5.0.1/grenade/flash_player
#
# @executed	as @e[tag=mgs.grenade] & at @s
#
# @within	mgs:v5.0.1/grenade/flash_check
#

# Tactical Mask perk (MP): greatly reduced flash — short blindness, no darkness, brief screen
execute if score @s mgs.mp.in_game matches 1 if score @s mgs.special.tactical_mask matches 1 run return run function mgs:v5.0.1/grenade/flash_player_masked
