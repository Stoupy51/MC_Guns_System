...
#> mgs:v5.0.1/maps/editor/give_tools
#
# @within	mgs:v5.0.1/maps/editor/enter
#			mgs:v5.0.1/maps/editor/invite_all [ as @a[scores={mgs.mp.map_edit=1}] ]
#			mgs:v5.0.1/maps/editor/save_only
#
...
# Coord stick utility
item replace entity @s inventory.23 with minecraft:warped_fungus_on_a_stick[minecraft:item_name=[{"text":"📐 ","color":"yellow","italic":false,"bold":true}, {"translate":"mgs.coord_stick"}],minecraft:custom_data={mgs:{coord_stick:true}},minecraft:item_model="minecraft:stick",minecraft:enchantment_glint_override=true]

# Mode-specific eggs
execute if score @s mgs.mp.map_disp matches 0 run function mgs:v5.0.1/maps/editor/give_tools/multiplayer
execute if score @s mgs.mp.map_disp matches 1 run function mgs:v5.0.1/maps/editor/give_tools/zombies
execute if score @s mgs.mp.map_disp matches 2 run function mgs:v5.0.1/maps/editor/give_tools/missions
