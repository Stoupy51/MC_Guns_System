...
#> mgs:v5.0.1/maps/editor/enter
#
# @within	mgs:v5.0.1/maps/editor/menu_entry_display {idx:$(idx),mode:$(mode)}"},"hover_event":{"action":"show_text","value":"Edit this map"}},{"translate":"mgs.edit"},"]"]," ",[{"text":"[","color":"red","click_event":{"action":"suggest_command","command":"/function mgs:v5.0.1/maps/editor/delete {idx:$(idx),mode:$(mode)}"},"hover_event":{"action":"show_text","value":"Delete this map"}},{"translate":"mgs.delete"},"]"]]
#
# @args		idx (unknown)
#			mode (unknown)
#
...
# Load map data
function mgs:v5.0.1/maps/editor/load_map_data with storage mgs:temp map_edit
...
function mgs:v5.0.1/shared/tp_to_position with storage mgs:temp _tp

# Summon markers for existing elements
function mgs:v5.0.1/maps/editor/summon_existing

# Give editor tools (dispatch by mode)
function mgs:v5.0.1/maps/editor/give_tools

# Initialize zombies element defaults (only for zombies mode)
execute if score @s mgs.mp.map_mode matches 1 run function mgs:v5.0.1/maps/editor/init_zb_defaults
...
tellraw @s [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],{"translate":"mgs.need_collaborators","color":"gray"},[{"text": "[", "color": "aqua", "click_event": {"action": "suggest_command", "command": "/function mgs:v5.0.1/maps/editor/invite_all"}, "hover_event": {"action": "show_text", "value": "Put all online players into this editor session"}}, "Invite All Players", "]"]]
tellraw @s [[{"text":"","color":"gold"},"[",{"translate":"mgs"},"] "],{"translate":"mgs.use","color":"gray"},[{"text": "[", "color": "green", "click_event": {"action": "suggest_command", "command": "/function mgs:v5.0.1/maps/editor/save_exit"}, "hover_event": {"action": "show_text", "value": "Save changes and exit editor"}}, "Save & Exit", "]"],{"text":" or "},[{"text": "[", "color": "red", "click_event": {"action": "suggest_command", "command": "/function mgs:v5.0.1/maps/editor/exit"}, "hover_event": {"action": "show_text", "value": "Discard changes and exit editor"}}, "Exit", "]"]]
