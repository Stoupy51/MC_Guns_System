...
#> mgs:v5.0.1/maps/editor/handle_config
#
...
execute unless data storage mgs:temp map_edit.map.default_enemy_function run data modify storage mgs:temp map_edit.map.default_enemy_function set value "mgs:v5.0.1/mob/default/level_1 {\"entity\":\"pillager\"}"
...
function mgs:v5.0.1/maps/editor/handle_config_default_btn with storage mgs:temp _cfg
...
execute if entity @e[tag=mgs.element.enemy,distance=..10] run function mgs:v5.0.1/maps/editor/handle_config_nearest_enemy_btn with storage mgs:temp _cfg
...
execute if entity @e[tag=mgs.element.start_command,distance=..10] run function mgs:v5.0.1/maps/editor/handle_config_nearest_start_command_btn with storage mgs:temp _cfg
...
execute if entity @e[tag=mgs.element.respawn_command,distance=..10] run function mgs:v5.0.1/maps/editor/handle_config_nearest_respawn_command_btn with storage mgs:temp _cfg
tellraw @p[tag=mgs.map_editor,distance=..6,sort=nearest] {"text":"============================================","color":"dark_gray"}
