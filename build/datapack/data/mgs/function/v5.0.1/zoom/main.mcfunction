...
execute unless data storage mgs:gun all.gun run return run function mgs:v5.0.1/zoom/check_slowness
...
execute if predicate mgs:v5.0.1/is_sneaking unless entity @s[tag=mgs.reloading] run scoreboard players set #is_sneaking mgs.data 1

# If already zoom and not sneaking, unzoom
execute if data storage mgs:gun all.stats.is_zoom if score #is_sneaking mgs.data matches 0 run return run function mgs:v5.0.1/zoom/remove

# If not zooming but sneaking, zoom
execute unless data storage mgs:gun all.stats.is_zoom if score #is_sneaking mgs.data matches 1 run return run function mgs:v5.0.1/zoom/set
...
execute if score @s mgs.zoom matches 1 unless score @s mgs.switch_cooldown > #total_tick mgs.data if score @s mgs.zoom_timer matches 6.. if score #scope_level mgs.data matches 3 at @s anchored eyes run particle minecraft:dust{color:[0.02,0.02,0.0],scale:0.01} ^ ^ ^0.1 0 0 0 0 1 force @s
execute if score @s mgs.zoom matches 1 unless score @s mgs.switch_cooldown > #total_tick mgs.data if score @s mgs.zoom_timer matches 6.. if score #scope_level mgs.data matches 4 at @s anchored eyes run particle minecraft:dust{color:[0.02,0.08,0.0],scale:0.01} ^ ^ ^0.1 0 0 0 0 1 force @s
execute if score @s mgs.zoom matches 1 unless score @s mgs.switch_cooldown > #total_tick mgs.data if score @s mgs.zoom_timer matches 6.. unless score #scope_level mgs.data matches 3 unless score #scope_level mgs.data matches 4 at @s anchored eyes run particle minecraft:dust{color:[0.02,0.25,0.0],scale:0.01} ^ ^ ^0.1 0 0 0 0 1 force @s

# Crosshair spread marker: spawn when NOT zooming to indicate accuracy via crosshair gap
execute unless score @s mgs.zoom matches 1 run function mgs:v5.0.1/zoom/crosshair_spread
