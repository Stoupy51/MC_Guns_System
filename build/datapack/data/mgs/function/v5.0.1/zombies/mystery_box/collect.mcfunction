...
execute if data storage mgs:zombies mystery_box.result.give_function run function mgs:v5.0.1/zombies/mystery_box/give_via_function
...
execute if data storage mgs:zombies mystery_box.result.weapon_id run function mgs:v5.0.1/zombies/mystery_box/capture_collected_name with storage mgs:zombies mystery_box.result
...
execute as @n[tag=mgs.mystery_box_active] at @s run function mgs:v5.0.1/zombies/feedback/sound_box_close

# Remove buyer tag
tag @s remove mgs.mb_buyer
