...
#> mgs:v5.0.1/zombies/mystery_box/show_result
#
# @within	mgs:v5.0.1/zombies/mystery_box/spin_tick
#

# If box will move, show teddy bear instead of weapon
execute if score #mb_will_move mgs.data matches 1 run return run function mgs:v5.0.1/zombies/mystery_box/show_bear_result

# Set display to final result
execute if data storage mgs:zombies mystery_box.result.weapon_id run function mgs:v5.0.1/zombies/mystery_box/show_result_weapon with storage mgs:zombies mystery_box.result
