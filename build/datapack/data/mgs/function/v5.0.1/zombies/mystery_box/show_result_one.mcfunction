...
execute if score @s mgs.mb.willmove matches 1 run return run function mgs:v5.0.1/zombies/mystery_box/show_bear_result
...
execute as @a[scores={mgs.zb.in_game=1}] if score @s mgs.mb.buying = #this_box mgs.data run function mgs:v5.0.1/zombies/mystery_box/pick_for_buyer

# All owned / empty pool: refund the buyer and cancel this pull
execute if score #mb_owned mgs.data matches 1 run function mgs:v5.0.1/zombies/mystery_box/result_all_owned
execute if score #mb_owned mgs.data matches 1 run return run function mgs:v5.0.1/zombies/mystery_box/reset_one

# Set this display to the final weapon and bake the result onto it for collect
execute if data storage mgs:zombies mystery_box.result.weapon_id run function mgs:v5.0.1/zombies/mystery_box/show_result_weapon_one with storage mgs:zombies mystery_box.result
