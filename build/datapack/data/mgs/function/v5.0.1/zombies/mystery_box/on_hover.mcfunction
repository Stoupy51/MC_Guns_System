...
# Active box mid-move
execute if score #mb_move_timer mgs.data matches 1.. if entity @e[tag=bs.interaction.target,tag=mgs.mystery_box_active] run return run function mgs:v5.0.1/zombies/mystery_box/hud_moving

# This box's pull state (at the box)
execute at @n[tag=bs.interaction.target] run function mgs:v5.0.1/zombies/mystery_box/hover_at_box
