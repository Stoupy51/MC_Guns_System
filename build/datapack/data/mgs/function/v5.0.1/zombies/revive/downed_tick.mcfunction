...
#> mgs:v5.0.1/zombies/revive/downed_tick
#
# @executed	at @s
#
# @within	mgs:v5.0.1/zombies/revive/tick [ at @s ]
#
...
execute if score #zb_reviving mgs.data matches 1 if entity @a[scores={mgs.zb.in_game=1,mgs.zb.downed=0},gamemode=!spectator,distance=..2.5,tag=mgs.perk.quick_revive] if score @s mgs.zb.revive_p matches 30.. run function mgs:v5.0.1/zombies/revive/revive_complete
execute if score #zb_reviving mgs.data matches 1 unless entity @a[scores={mgs.zb.in_game=1,mgs.zb.downed=0},gamemode=!spectator,distance=..2.5,tag=mgs.perk.quick_revive] if score @s mgs.zb.revive_p matches 60.. run function mgs:v5.0.1/zombies/revive/revive_complete

# Bleed out: time's up
execute if score @s mgs.zb.bleed matches ..0 run function mgs:v5.0.1/zombies/revive/bleed_out
...
execute if score #zb_reviving mgs.data matches 0 unless entity @a[scores={mgs.zb.in_game=1,mgs.zb.downed=0},gamemode=!spectator] run function mgs:v5.0.1/zombies/revive/bleed_out
