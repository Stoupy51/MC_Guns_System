...
#> mgs:v5.0.1/zombies/doors/on_right_click
#
# @executed	as @e[tag=mgs.door_new]
#
# @within	mgs:v5.0.1/zombies/doors/setup_iter {run:"function mgs:v5.0.1/zombies/doors/on_right_click",executor:"source"} [ as @e[tag=mgs.door_new] ]
#
...
execute unless score @s mgs.zb.points >= #door_price mgs.data run return run function mgs:v5.0.1/zombies/doors/deny_not_enough_points
...
execute if entity @e[tag=bs.interaction.target,tag=mgs.door_back] run function mgs:v5.0.1/zombies/doors/get_hover_name_back with storage mgs:temp _door_hover
execute unless entity @e[tag=bs.interaction.target,tag=mgs.door_back] run function mgs:v5.0.1/zombies/doors/get_hover_name with storage mgs:temp _door_hover

# Open all doors with matching link_id
execute as @e[tag=mgs.door] if score @s mgs.zb.door.link = #door_link mgs.data at @s run function mgs:v5.0.1/zombies/doors/open_one
