...
#> mgs:v5.0.1/zombies/doors/setup_iter
#
# @within	mgs:v5.0.1/zombies/doors/setup
#			mgs:v5.0.1/zombies/doors/setup_iter
#
...
function mgs:v5.0.1/zombies/doors/place_at with storage mgs:temp _door
...
function mgs:v5.0.1/zombies/doors/store_name with storage mgs:temp _door_name

# Register Bookshelf events
execute as @e[tag=mgs.door_new] run function #bs.interaction:on_right_click {run:"function mgs:v5.0.1/zombies/doors/on_right_click",executor:"source"}
execute as @e[tag=mgs.door_new] run function #bs.interaction:on_hover {run:"function mgs:v5.0.1/zombies/doors/on_hover",executor:"source"}
tag @e[tag=mgs.door_new] remove mgs.door_new

# Continue iteration
data remove storage mgs:temp _door_iter[0]
execute if data storage mgs:temp _door_iter[0] run function mgs:v5.0.1/zombies/doors/setup_iter
