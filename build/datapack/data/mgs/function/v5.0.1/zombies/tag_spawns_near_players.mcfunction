...
#> mgs:v5.0.1/zombies/tag_spawns_near_players
#
# @executed	as @e[tag=...,limit=24,sort=random] & at @s
#
# @within	mgs:v5.0.1/zombies/on_stuck_zombie
#			mgs:v5.0.1/zombies/spawn_zombie
#

scoreboard players set #zb_near_found mgs.data 0

# First pass: 32 blocks from any alive player
execute as @a[scores={mgs.zb.in_game=1},gamemode=!spectator] at @s run function mgs:v5.0.1/zombies/tag_zb_near_32

# Second pass: 64 blocks if none found
execute if score #zb_near_found mgs.data matches 0 as @a[scores={mgs.zb.in_game=1},gamemode=!spectator] at @s run function mgs:v5.0.1/zombies/tag_zb_near_64
