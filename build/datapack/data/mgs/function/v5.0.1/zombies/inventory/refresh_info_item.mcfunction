...
#
# @within	mgs:v5.0.1/zombies/game_tick [ as @a[scores={mgs.zb.in_game=1},gamemode=!spectator] ]
#			mgs:v5.0.1/zombies/inventory/give_starting_loadout
#			mgs:v5.0.1/zombies/inventory/recreate_critical_items
#
...
function mgs:v5.0.1/zombies/inventory/build_info_lore with storage mgs:temp info
...
execute if score @s mgs.zb.perk.juggernog matches 1 run data modify storage mgs:temp info.lore append value {"text":"\u2022 Juggernog","color":"dark_red","italic":false}
...
execute if score @s mgs.zb.perk.double_tap matches 1 run data modify storage mgs:temp info.lore append value {"text":"\u2022 Double Tap","color":"gold","italic":false}
...
execute if score @s mgs.zb.perk.mule_kick matches 1 run data modify storage mgs:temp info.lore append value {"text":"\u2022 Mule Kick","color":"gold","italic":false}
execute if score @s mgs.zb.perk.stamin_up matches 1 run data modify storage mgs:temp info.lore append value {"text":"\u2022 Stamin-Up","color":"yellow","italic":false}

function mgs:v5.0.1/zombies/inventory/refresh_info_item_render with storage mgs:temp info
function mgs:v5.0.1/zombies/inventory/apply_slot_tag {slot:"hotbar.8",group:"hotbar",index:8}
