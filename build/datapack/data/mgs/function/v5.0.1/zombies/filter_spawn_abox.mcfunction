...
#> mgs:v5.0.1/zombies/filter_spawn_abox
#
# @executed	as @e[tag=mgs.zb_near]
#
# @within	mgs:v5.0.1/zombies/spawn_zombie [ as @e[tag=mgs.zb_near] ]
#

data modify storage mgs:temp _abox_chk set from entity @s data.abox
scoreboard players set #abox_ok mgs.data 0
function mgs:v5.0.1/zombies/test_spawn_abox with storage mgs:temp _abox_chk
