...
function mgs:v5.0.1/shared/load_base_coordinates {mode:"zombies"}
...
execute if score #zb_has_bounds mgs.data matches 1 run function mgs:v5.0.1/shared/load_bounds {mode:"zombies"}

# Forceload the area (only if bounds defined)
execute if score #zb_has_bounds mgs.data matches 1 run function mgs:v5.0.1/shared/forceload_area
...
execute as @a[scores={mgs.zb.in_game=1}] run function mgs:v5.0.1/shared/tp_to_position with storage mgs:temp _tp
...
schedule function mgs:v5.0.1/zombies/preload_complete 20t
