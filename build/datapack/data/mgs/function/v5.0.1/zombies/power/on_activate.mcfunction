...
#> mgs:v5.0.1/zombies/power/on_activate
#
# @executed	as @e[tag=_pw_new]
#
# @within	mgs:v5.0.1/zombies/power/place_at {run:"function mgs:v5.0.1/zombies/power/on_activate",executor:"source"} [ as @e[tag=_pw_new] ]
#
...
execute if score #zb_power mgs.data matches 1 run return run function mgs:v5.0.1/zombies/power/deny_already_on
