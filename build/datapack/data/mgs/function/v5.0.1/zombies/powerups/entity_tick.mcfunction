...
#> mgs:v5.0.1/zombies/powerups/entity_tick
#
# @executed	as @e[tag=mgs.pu_item] & at @s
#
# @within	mgs:v5.0.1/zombies/game_tick [ as @e[tag=mgs.pu_item] & at @s ]
#
...
execute if score @s mgs.zb.pu.timer matches ..0 run return run function mgs:v5.0.1/zombies/powerups/expire

# Blink warning in the last 10 seconds
execute if score @s mgs.zb.pu.timer matches 1..199 run function mgs:v5.0.1/zombies/powerups/blink_tick
