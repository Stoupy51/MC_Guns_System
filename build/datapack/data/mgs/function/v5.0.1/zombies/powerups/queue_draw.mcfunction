...
# Refill if empty
execute if score #pu_q_len mgs.data matches ..0 run function mgs:v5.0.1/zombies/powerups/queue_refill
...
execute if score #pu_q_len mgs.data matches 11 store result score #pu_q_idx mgs.data run random value 0..10
...
function mgs:v5.0.1/zombies/powerups/queue_extract with storage mgs:temp _pu_q
