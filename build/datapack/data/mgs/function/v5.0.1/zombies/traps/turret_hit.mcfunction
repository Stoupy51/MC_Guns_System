...
execute if entity @s[tag=mgs.zombie_round] run data modify storage mgs:temp _trap_dmg.type set value "minecraft:mob_projectile"
...
execute if entity @s[type=player,gamemode=!creative,gamemode=!spectator] if score @s mgs.zb.in_game matches 1.. run damage @s 2 minecraft:mob_projectile
