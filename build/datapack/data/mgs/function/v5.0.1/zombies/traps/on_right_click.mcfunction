...
#> mgs:v5.0.1/zombies/traps/on_right_click
#
# @executed	as @e[tag=mgs._trap_new_bs]
#
# @within	mgs:v5.0.1/zombies/traps/setup_iter {run:"function mgs:v5.0.1/zombies/traps/on_right_click",executor:"source"} [ as @e[tag=mgs._trap_new_bs] ]
#
...
execute if score #trap_power mgs.data matches 1 unless score #zb_power mgs.data matches 1 run return run function mgs:v5.0.1/zombies/traps/deny_requires_power
...
execute as @e[tag=mgs.trap_center] if score @s mgs.zb.trap.id = #trap_id mgs.data if score @s mgs.zb.trap.timer matches 0 unless score @s mgs.zb.trap.cd > #total_tick mgs.data run scoreboard players set #trap_ready mgs.data 1
execute unless score #trap_ready mgs.data matches 1 run return run function mgs:v5.0.1/zombies/traps/deny_not_ready

# Check price
execute store result score #trap_price mgs.data run scoreboard players get @n[tag=bs.interaction.target] mgs.zb.trap.price
execute unless score @s mgs.zb.points >= #trap_price mgs.data run return run function mgs:v5.0.1/zombies/traps/deny_not_enough_points
...
function mgs:v5.0.1/zombies/feedback/sound_announce
