...
#> mgs:v5.0.1/zombies/preload_complete
#
# @within	mgs:v5.0.1/zombies/start 20t [ scheduled ]
#
...
execute if data storage mgs:zombies game.map.out_of_bounds run function mgs:v5.0.1/shared/summon_oob {mode:"zombies"}

# Summon spawn point markers for players
function mgs:v5.0.1/zombies/summon_spawns
...
execute if data storage mgs:zombies game.map.pap_machines[0] run function mgs:v5.0.1/zombies/pap/setup

# Setup barriers
execute if data storage mgs:zombies game.map.barriers[0] run function mgs:v5.0.1/zombies/barriers/setup

# Setup power switches
execute if data storage mgs:zombies game.map.power_switch[0] run function mgs:v5.0.1/zombies/power/setup

# Setup doors
execute if data storage mgs:zombies game.map.doors[0] run function mgs:v5.0.1/zombies/doors/setup

# Setup wallbuys
execute if data storage mgs:zombies game.map.wallbuys[0] run function mgs:v5.0.1/zombies/wallbuys/setup
