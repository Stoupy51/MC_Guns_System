...
#> mgs:v5.0.1/zombies/barriers/intact_tick
#
...
function mgs:v5.0.1/zombies/barriers/freeze_zombies with storage mgs:temp _btick

# Handle remove timer or find a new remover (both macros using radius)
execute if score @s mgs.zb.barrier.r_timer matches 1.. run function mgs:v5.0.1/zombies/barriers/handle_removing with storage mgs:temp _btick
execute if score @s mgs.zb.barrier.r_timer matches 0 if score @s mgs.zb.barrier.state matches 0 run function mgs:v5.0.1/zombies/barriers/find_remover with storage mgs:temp _btick
