...
#> mgs:v5.0.1/zombies/pap/setup_iter
#
# @within	mgs:v5.0.1/zombies/pap/setup
#			mgs:v5.0.1/zombies/pap/setup_iter
#
...
function mgs:v5.0.1/zombies/pap/place_at with storage mgs:temp _pap_place
...
execute as @n[tag=mgs.pap_new] run function #bs.interaction:on_right_click {run:"function mgs:v5.0.1/zombies/pap/on_right_click",executor:"source"}
execute as @n[tag=mgs.pap_new] run function #bs.interaction:on_hover {run:"function mgs:v5.0.1/zombies/pap/on_hover",executor:"source"}
...
execute if data storage mgs:temp _pap_disp{item_model:""} run data modify storage mgs:temp _pap_disp.item_model set value "minecraft:netherite_block"
...
execute as @n[tag=mgs.pap_new] at @s run function mgs:v5.0.1/zombies/display/summon_machine_display with storage mgs:temp _pap_disp
...
function mgs:v5.0.1/zombies/pap/store_data with storage mgs:temp _pap_store

tag @n[tag=mgs.pap_new] remove mgs.pap_new

# Continue iteration
data remove storage mgs:temp _pap_iter[0]
execute if data storage mgs:temp _pap_iter[0] run function mgs:v5.0.1/zombies/pap/setup_iter
