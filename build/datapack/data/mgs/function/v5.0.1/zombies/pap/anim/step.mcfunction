...
#> mgs:v5.0.1/zombies/pap/anim/step
#
# @executed	at @s
#
...
execute if score @s mgs.pap_anim matches 298 run function mgs:v5.0.1/zombies/pap/anim/trigger_going_in

# Phase: GOING IN (timer 281..297)
execute if score @s mgs.pap_anim matches 281..297 run function mgs:v5.0.1/zombies/pap/anim/going_in

# Trigger: weapon fully in at timer=280 — start inside processing
execute if score @s mgs.pap_anim matches 280 run function mgs:v5.0.1/zombies/pap/anim/trigger_inside

# Phase: INSIDE (timer 225..279)
execute if score @s mgs.pap_anim matches 225..279 run function mgs:v5.0.1/zombies/pap/anim/inside

# Trigger: apply scope+camo cosmetics at midpoint of inside phase (timer=252)
execute if score @s mgs.pap_anim matches 252 run function mgs:v5.0.1/zombies/pap/anim/apply_cosmetics

# Trigger: start coming-out interpolation at timer=225
execute if score @s mgs.pap_anim matches 225 run function mgs:v5.0.1/zombies/pap/anim/trigger_coming_out

# Phase: COMING OUT (timer 206..219)
execute if score @s mgs.pap_anim matches 206..219 run function mgs:v5.0.1/zombies/pap/anim/coming_out
...
execute if score @s mgs.pap_anim matches 1..205 if score #pap_t mgs.data matches 0 run function mgs:v5.0.1/zombies/feedback/sound_pap_retreat_loop

# Retreat finished at timer=0 — weapon is lost
execute if score @s mgs.pap_anim matches 0 run function mgs:v5.0.1/zombies/pap/anim/retreat_finish
