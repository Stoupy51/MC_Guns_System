...
#
# @within	mgs:v5.0.1/zombies/pap/setup_iter {run:"function mgs:v5.0.1/zombies/pap/on_right_click",executor:"source"} [ as @n[tag=mgs.pap_new] ]
#
...
execute if score @n[tag=bs.interaction.target] mgs.pap_anim matches 1..205 run return run function mgs:v5.0.1/zombies/pap/anim/collect
# If machine is going-in, inside, or coming-out (not yet collectible), deny
execute if score @n[tag=bs.interaction.target] mgs.pap_anim matches 206.. run return run function mgs:v5.0.1/zombies/pap/anim/deny_processing
...
execute if score #pap_power mgs.data matches 1 unless score #zb_power mgs.data matches 1 run return run function mgs:v5.0.1/zombies/pap/deny_requires_power
...
execute unless score #pap_sel mgs.data matches 1..3 run return run function mgs:v5.0.1/zombies/pap/deny_hold_weapon_slot
...
execute unless score #pap_is_gun mgs.data matches 1 run return run function mgs:v5.0.1/zombies/pap/deny_not_gun

# Extract selected item data
function mgs:v5.0.1/zombies/pap/extract_selected with storage mgs:temp _pap

# Guard: selected weapon must provide PAP data in its own stats
execute unless data storage mgs:temp _pap_extract.stats.pap_stats run return run function mgs:v5.0.1/zombies/pap/deny_not_supported
...
function mgs:v5.0.1/zombies/pap/compute_max_level
execute if score #pap_next mgs.data > #pap_max mgs.data run return run function mgs:v5.0.1/zombies/pap/repap_scope_only with storage mgs:temp _pap
...
function mgs:v5.0.1/zombies/pap/apply_runtime_overrides
...
function mgs:v5.0.1/zombies/pap/randomize_scope with storage mgs:temp _pap_extract.stats

# Randomize weapon camo (applied after scope, so camo appends to the scoped weapon id)
function mgs:v5.0.1/zombies/pap/randomize_camo with storage mgs:temp _pap_extract.stats
...
function mgs:v5.0.1/zombies/pap/anim/store_cosmetics with storage mgs:temp _pap_cosm_store
...
execute if data storage mgs:temp _pap_extract.stats.pap_stats.pap_name run function mgs:v5.0.1/zombies/pap/resolve_runtime_name
...
function mgs:v5.0.1/zombies/pap/lookup_machine with storage mgs:temp _pap_hover
function mgs:v5.0.1/zombies/pap/pap_chat_message
...
function mgs:v5.0.1/zombies/pap/apply_to_slot with storage mgs:temp _pap
function mgs:v5.0.1/zombies/pap/pap_upgrade_magazines with storage mgs:temp _pap_extract.stats
...
execute as @n[tag=bs.interaction.target] at @s run function mgs:v5.0.1/zombies/pap/anim/start with storage mgs:temp _pap
