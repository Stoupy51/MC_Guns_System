...
#> mgs:v5.0.1/zombies/wallbuys/setup_iter
#
# @within	mgs:v5.0.1/zombies/wallbuys/setup
#			mgs:v5.0.1/zombies/wallbuys/setup_iter
#
...
function mgs:v5.0.1/zombies/wallbuys/place_at with storage mgs:temp _wb
...
execute as @n[tag=mgs.wb_new] run function #bs.interaction:on_right_click {run:"function mgs:v5.0.1/zombies/wallbuys/on_right_click",executor:"source"}
execute as @n[tag=mgs.wb_new] run function #bs.interaction:on_hover {run:"function mgs:v5.0.1/zombies/wallbuys/on_hover",executor:"source"}
tag @n[tag=mgs.wb_new] remove mgs.wb_new

# Set item on display entity
function mgs:v5.0.1/zombies/wallbuys/set_display_item with storage mgs:temp _wb
...
function mgs:v5.0.1/zombies/wallbuys/store_data with storage mgs:temp _wb_store

tag @e[tag=mgs.wb_new_display] remove mgs.wb_new_display

# Continue iteration
data remove storage mgs:temp _wb_iter[0]
execute if data storage mgs:temp _wb_iter[0] run function mgs:v5.0.1/zombies/wallbuys/setup_iter
