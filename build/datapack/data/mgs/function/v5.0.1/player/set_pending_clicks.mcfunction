...
#> mgs:v5.0.1/player/set_pending_clicks
#
# @executed	as the player & at current position
#
# @within	advancement mgs:v5.0.1/right_click
#

# Revoke advancement
advancement revoke @s only mgs:v5.0.1/right_click
...
execute if score @s mgs.burst_count matches 1.. run function mgs:v5.0.1/utils/copy_gun_data
execute if score @s mgs.burst_count matches 1.. if data storage mgs:gun all.stats{fire_mode:"burst"} run function mgs:v5.0.1/player/check_mid_burst
