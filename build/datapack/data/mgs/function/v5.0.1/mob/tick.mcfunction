...
#
# @within	mgs:v5.0.1/tick [ as @e[tag=mgs.armed] & at @s ]
#

# Initialize mob if not yet initialized
execute unless entity @s[tag=mgs.mob_init] run function mgs:v5.0.1/mob/init
...
execute if score @s mgs.mob.timer matches 0 if entity @s[tag=mgs.mob_sleeping] run function mgs:v5.0.1/mob/wake_up
execute if score @s mgs.mob.timer matches 0 unless entity @s[tag=mgs.mob_sleeping] unless score @s mgs.mob.sleep_time matches 0 run function mgs:v5.0.1/mob/go_sleep
...
function mgs:v5.0.1/mob/copy_gun_data
...
execute anchored eyes facing entity @n[tag=mgs.target] feet run function mgs:v5.0.1/mob/fire_weapon
