...
execute if data storage stoupgun:gun all.stats unless data storage stoupgun:gun all.stats.weapon_id run function stoupgun:v5.0.0/switch/set_weapon_id

# If last_selected is different from this one, set cooldown
scoreboard players set #current_id stoupgun.data 0
execute store result score #current_id stoupgun.data run data get storage stoupgun:gun all.stats.weapon_id
