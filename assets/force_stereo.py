
# Script that forces all .ogg files to use stereo channel.
# Requires ffmpeg to be installed.

import os
import subprocess
from multiprocessing import Pool


def convert_file(args: tuple[str, str]) -> None:
	src, dst = args
	previous_size: int = os.path.getsize(src)
	subprocess.run(["ffmpeg", "-i", src, "-b:a", "64k", "-ac", "2", dst], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

	# Remove original & rename temp
	file_size: int = os.path.getsize(dst)
	if file_size < previous_size:
		os.remove(src)
		os.rename(dst, src)
		print(f"Stereo file '{src}' got from {previous_size} to {file_size} bytes")


if __name__ == "__main__":
	py_path: str = os.path.dirname(os.path.abspath(__file__))

	files_to_compress: list[tuple[str, str]] = []
	for root, _, files in os.walk(py_path):
		for file in files:
			if file.endswith(".ogg"):
				src: str = f"{root}/{file}"
				dst: str = src.replace(".ogg", ".temp.ogg")
				files_to_compress.append((src, dst))

	# Compress
	cpu_count: int = (os.cpu_count() or 1) // 2 + 1
	with Pool(processes=cpu_count) as pool:
		pool.map(convert_file, files_to_compress)
	print("Conversion finished!")

