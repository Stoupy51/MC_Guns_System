...
- A pass must not sample the target it writes. Ping-pong through a second target, as vanilla `blur`
  does with `swap`.
...
What it does not solve: a weapon firing every tick can never toggle, because you need one tick
applied and one tick off to re-arm. The pack already gates flashes behind a 3-tick cooldown
(`{ns}.last_muzzle_flash`, [shaders.py](src/functional/shaders.py)), so this is not a real
constraint today, but it is the ceiling: **one flash per 2 ticks, 10 per second**.
...
which routes hand and 3D HUD depth into a separate target and adds a depth integration pass. The
payoff is that `minecraft:main`'s depth becomes clean world depth, with the gun no longer in it.
`FLASH_FSH` wants exactly that.
...
- [ ] No pass sampling its own output.
- [ ] Nothing assumes a non-persistent target's previous contents.
- [ ] `posteffect clear` on player join and on game end. Not needed on respawn.
