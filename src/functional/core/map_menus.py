...
def write_shared_map_menus() -> None:
		...
		## Map select iterator: inject the mode into each entry, then call select_entry.
		## The caller sets _map_iter, _map_select_mode, and pre-builds the base dialog with an empty actions list.
		## select_entry appends one action button per map, then the caller shows the completed dialog.
