...
def generate_missions_maps() -> None:
	...
	## Map loading with search (same pattern as multiplayer)
	write_versioned_function("maps/missions/load", f"""
# Load a missions map by id
# Usage: /function {ns}:v{version}/maps/missions/load {{id:"map_id",override:{{}}}}
...
data modify storage {ns}:temp map_load.search set from storage {ns}:maps missions

# Initialize search
scoreboard players set #map_load_idx {ns}.data 0
scoreboard players set #map_load_found {ns}.data 0
function {ns}:v{version}/maps/missions/find_map
""")

	## Find map by ID (iterates list)
	write_versioned_function("maps/missions/find_map", f"""
...
function {ns}:v{version}/maps/missions/check_map_id with storage {ns}:temp map_load.target
...
""")
	...
	write_versioned_function("maps/missions/check_map_id", f"""
