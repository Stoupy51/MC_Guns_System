...
def generate_menus() -> None:
	...
	setup_actions = [
		Dialogs.dialog_show_btn(f"{ns}:multiplayer/setup/gamemode", "🎮 Gamemode", "Choose the multiplayer gamemode"),
		Dialogs.dialog_show_btn(f"{ns}:multiplayer/setup/score_limit", "🏆 Score Limit", "Set the score needed to win"),
		Dialogs.dialog_show_btn(f"{ns}:multiplayer/setup/time_limit", "⏱ Time Limit", "Set the match time limit"),
		Dialogs.dialog_run_btn("🗺 Select Map", f"/function {ns}:v{version}/multiplayer/map_select", "Browse and select a map", "aqua"),
		Dialogs.dialog_run_btn("▶ START", f"/function {ns}:v{version}/multiplayer/start", "Start the match", "green"),
		Dialogs.dialog_run_btn("■ STOP", f"/function {ns}:v{version}/multiplayer/stop", "Stop the match", "red"),
		Dialogs.dialog_run_btn("⚔ Classes", f"/function {ns}:v{version}/multiplayer/select_class", "Select your class", "aqua"),
		Dialogs.dialog_run_btn("+ Join", f"/function {ns}:v{version}/multiplayer/join_game", "Join the ongoing game as a late joiner", "yellow"),
		Dialogs.dialog_run_btn("Red Team", f"/function {ns}:v{version}/multiplayer/join_red", "Join Red Team", "red"),
		Dialogs.dialog_run_btn("Blue Team", f"/function {ns}:v{version}/multiplayer/join_blue", "Join Blue Team", "blue"),
		Dialogs.dialog_run_btn("Auto Team", f"/execute as @a[sort=random] run function {ns}:v{version}/multiplayer/auto_assign_team", "Auto-balance across Red/Blue (in FFA, seats everyone on the single FFA team)", "yellow"),
		Dialogs.dialog_run_btn("👥 Manage Players", f"/function {ns}:v{version}/players/list_multiplayer", "Assign players to Red/Blue teams", "dark_aqua"),
	]
	Dialogs.register_dialog("multiplayer/setup", {
