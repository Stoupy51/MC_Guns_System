...
def generate_menus() -> None:
	...
	write_versioned_function("multiplayer/setup", f"dialog show @s {ns}:multiplayer/setup")
...
execute unless data storage {ns}:temp dialog.actions[0] run data modify storage {ns}:temp dialog.actions append value {{label:{{text:"No maps registered",color:"red"}},tooltip:{{text:"Create one in the map editor first"}},action:{{type:"show_dialog",dialog:"{ns}:multiplayer/setup"}}}}
