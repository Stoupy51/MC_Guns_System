...
class FunctionalHelpers:
	""" Shared helpers for the functional generators. """

	SPECIAL_SCORES: ClassVar[dict[str, str]] = {
	...
	def special_objectives_lines(ns: str) -> str:
		...
		parts: list[str] = []
		for name, comment in FunctionalHelpers.SPECIAL_SCORES.items():
			if comment:
	...
	def reset_special_scores_lines(ns: str, selector: str) -> str:
		...
		return "\n".join(f"scoreboard players set {selector} {ns}.special.{name} 0" for name in FunctionalHelpers.SPECIAL_SCORES)
...
""".strip())
		if normalize_legacy:
			parts.append(FunctionalHelpers.normalize_map_command_lines(ns, mode))
...
""".strip()
	...
	def register_dialog(dialog_id: str, data: JsonDict, wrapper: bool = True) -> None:
		...
		if wrapper:
			write_versioned_function(f"dialogs/{dialog_id}", f"dialog show @s {FunctionalHelpers.dialog_ref(dialog_id)}")
	...
	def dialog_back_action(dialog_id: str, label: str = "◀ Back", tooltip: str = "Return to the previous menu") -> JsonDict:
		...
		return {
			"label": FunctionalHelpers.split_emoji(label, color="gray"),
			"tooltip": {"text": tooltip},
			"action": {"type": "minecraft:show_dialog", "dialog": FunctionalHelpers.dialog_ref(dialog_id)},
		}
	...
	def dialog_show_btn(dialog_reference: str, label: str, hover: str, color: str | None = None) -> JsonDict:
		...
		label_component: Any = FunctionalHelpers.split_emoji(label, color=color) if color else FunctionalHelpers.split_emoji(label)
		dialog_id: str = dialog_reference.split(":", 1)[-1]
		return {"label": label_component, "tooltip": {"text": hover}, "action": {"type": "minecraft:show_dialog", "dialog": FunctionalHelpers.dialog_ref(dialog_id)}}
	...
	def dialog_run_btn(label: str, command: str, hover: str, color: str = "green") -> JsonDict:
		...
		return {"label": FunctionalHelpers.split_emoji(label, color=color), "tooltip": {"text": hover}, "action": {"type": "run_command", "command": command}}
	...
	def register_value_picker(dialog_id: str, title: str, desc: str, options: list[tuple[str, str, str, str]], back_dialog: str) -> None:
		...
		FunctionalHelpers.register_dialog(dialog_id, {
			...
			"pause": False,
			"after_action": "none",
			"exit_action": FunctionalHelpers.dialog_back_action(back_dialog),
		})
