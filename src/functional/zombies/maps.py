...
def generate_zombies_maps() -> None:
	...
	## Map loading with search (same pattern as missions/multiplayer)
	write_versioned_function("maps/zombies/load", f"""
# Load a zombies map by id
# Usage: /function {ns}:v{version}/maps/zombies/load {{id:"map_id",override:{{}}}}
...
data modify storage {ns}:temp map_load.search set from storage {ns}:maps zombies

# Initialize search
scoreboard players set #map_load_idx {ns}.data 0
scoreboard players set #map_load_found {ns}.data 0
function {ns}:v{version}/maps/zombies/find_map
""")

	## Find map by ID (iterates list)
	write_versioned_function("maps/zombies/find_map", f"""
...
function {ns}:v{version}/maps/zombies/check_map_id with storage {ns}:temp map_load.target
...
""")
	...
	## Check if current element's ID matches
	write_versioned_function("maps/zombies/check_map_id", f"""
