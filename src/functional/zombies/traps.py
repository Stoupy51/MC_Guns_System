...
scoreboard players set #_trap_counter {ns}.data 0
...
scoreboard players add #_trap_counter {ns}.data 1

# Read trap center position (relative) and convert to absolute
execute store result score #_tx {ns}.data run data get storage {ns}:temp _trap_iter[0].pos[0]
execute store result score #_ty {ns}.data run data get storage {ns}:temp _trap_iter[0].pos[1]
execute store result score #_tz {ns}.data run data get storage {ns}:temp _trap_iter[0].pos[2]
scoreboard players operation #_tx {ns}.data += #gm_base_x {ns}.data
scoreboard players operation #_ty {ns}.data += #gm_base_y {ns}.data
scoreboard players operation #_tz {ns}.data += #gm_base_z {ns}.data

# Compute interaction entity position (trap center + offset_pos)
execute store result score #_tix {ns}.data run data get storage {ns}:temp _trap_iter[0].offset_pos[0]
execute store result score #_tiy {ns}.data run data get storage {ns}:temp _trap_iter[0].offset_pos[1]
execute store result score #_tiz {ns}.data run data get storage {ns}:temp _trap_iter[0].offset_pos[2]
scoreboard players operation #_tix {ns}.data += #_tx {ns}.data
scoreboard players operation #_tiy {ns}.data += #_ty {ns}.data
scoreboard players operation #_tiz {ns}.data += #_tz {ns}.data

# Store positions for macros
execute store result storage {ns}:temp _trap.cx int 1 run scoreboard players get #_tx {ns}.data
execute store result storage {ns}:temp _trap.cy int 1 run scoreboard players get #_ty {ns}.data
execute store result storage {ns}:temp _trap.cz int 1 run scoreboard players get #_tz {ns}.data
execute store result storage {ns}:temp _trap.ix int 1 run scoreboard players get #_tix {ns}.data
execute store result storage {ns}:temp _trap.iy int 1 run scoreboard players get #_tiy {ns}.data
execute store result storage {ns}:temp _trap.iz int 1 run scoreboard players get #_tiz {ns}.data

# Summon entities
function {ns}:v{version}/zombies/traps/place_at with storage {ns}:temp _trap

# Set scoreboards on interaction entity
scoreboard players operation @n[tag=_trap_new_i] {ns}.zb.trap.id = #_trap_counter {ns}.data
...
scoreboard players operation @n[tag=_trap_new_m] {ns}.zb.trap.id = #_trap_counter {ns}.data
...
execute store result score #_tr_x {ns}.data run data get storage {ns}:temp _trap_iter[0].effect_radius[0]
execute store result score #_tr_y {ns}.data run data get storage {ns}:temp _trap_iter[0].effect_radius[1]
execute store result score #_tr_z {ns}.data run data get storage {ns}:temp _trap_iter[0].effect_radius[2]
scoreboard players operation #_tr_max {ns}.data = #_tr_x {ns}.data
execute if score #_tr_y {ns}.data > #_tr_max {ns}.data run scoreboard players operation #_tr_max {ns}.data = #_tr_y {ns}.data
execute if score #_tr_z {ns}.data > #_tr_max {ns}.data run scoreboard players operation #_tr_max {ns}.data = #_tr_z {ns}.data
scoreboard players operation @n[tag=_trap_new_m] {ns}.zb.trap.radius = #_tr_max {ns}.data
...
execute store result score #_trap_power {ns}.data run scoreboard players get @n[tag=bs.interaction.target] {ns}.zb.trap.power
execute if score #_trap_power {ns}.data matches 1 unless score #zb_power {ns}.data matches 1 run return run tellraw @s [{MGS_TAG},{{"text":" ⚡ Requires power!","color":"red"}}]

# Get trap ID
execute store result score #_trap_id {ns}.data run scoreboard players get @n[tag=bs.interaction.target] {ns}.zb.trap.id

# Check if trap is ready (not active, not on cooldown)
scoreboard players set #_trap_ready {ns}.data 0
execute as @e[tag={ns}.trap_center] if score @s {ns}.zb.trap.id = #_trap_id {ns}.data if score @s {ns}.zb.trap.timer matches 0 if score @s {ns}.zb.trap.cd matches 0 run scoreboard players set #_trap_ready {ns}.data 1
execute unless score #_trap_ready {ns}.data matches 1 run return run tellraw @s [{MGS_TAG},{{"text":" Trap not ready!","color":"yellow"}}]

# Check price
execute store result score #_trap_price {ns}.data run scoreboard players get @n[tag=bs.interaction.target] {ns}.zb.trap.price
execute unless score @s {ns}.zb.points >= #_trap_price {ns}.data run return run tellraw @s [{MGS_TAG},{{"text":" Not enough points!","color":"red"}}]

# Deduct points
scoreboard players operation @s {ns}.zb.points -= #_trap_price {ns}.data

# Activate trap (set timer = duration on the marker)
execute as @e[tag={ns}.trap_center] if score @s {ns}.zb.trap.id = #_trap_id {ns}.data run scoreboard players operation @s {ns}.zb.trap.timer = @s {ns}.zb.trap.dur
...
title @s subtitle [{{"text":"Cost: ","color":"gray"}},{{"score":{{"name":"#_trap_price","objective":"{ns}.data"}},"color":"yellow"}},{{"text":" points","color":"gray"}}]
""")
