...
class KickGenerator(McfunctionGenerator):
    """ Generates the weapon recoil ("kick") system: a dispatcher that picks a kick
    strength from the held weapon's stats and per-strength rotation tables. """
...
""")
        ...
        for i, kicks in enumerate(all_kicks):
            ...
            self.func(f"kicks/type_{i}", content)


# Main function
def main() -> None:
    """ Module-level entry point (preserved signature); delegates to :class:`KickGenerator`. """
    KickGenerator()()
